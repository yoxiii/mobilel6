...
async def async_setup_entry(
    ...
    elif device.api.type in {"SP2", "SP2S", "SP3", "SP3S", "SP4B"}:
        switches.append(BroadlinkSP2Switch(device))

    elif device.api.type in {"SP4"}:
        switches.append(BroadlinkSP4Switch(device))
...
class BroadlinkSP4Switch(BroadlinkSP1Switch):
    ...
    def __init__(self, device, *args, **kwargs):
        ...
        super().__init__(device, *args, **kwargs)
        self._attr_is_on = self._coordinator.data["pwr"]
#        self._attr_is_lock = self._coordinator.data["childlock"]
